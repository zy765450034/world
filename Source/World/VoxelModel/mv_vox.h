...
public :
	...
	bool ReadModelFile( const char *path ) {
		...
		file.Seek( mainChunk.contentSize, SEEK_CUR );
		//std::vector< MV_Voxel > voxels;
		// read children chunks
		while ( file.Position() < mainChunk.end ) {
			// read chunk header
			chunk_t sub;
			ReadChunk( file, sub );
	...
	void ReadChunk( MV_ReadFile &file, chunk_t &chunk ) {
		// read chunk header
		chunk.id = file.ReadInt();
		chunk.contentSize  = file.ReadInt();
		chunk.childrenSize = file.ReadInt();

		// end of chunk : used for skipping the whole chunk
		chunk.end = file.Position() + chunk.contentSize + chunk.childrenSize;
